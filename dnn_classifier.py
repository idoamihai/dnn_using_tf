...
class dnn():
    ...
    def test_score(self,x_train,y_train,x_test,y_test,example_rows,num_labels,**kwargs):
        ...
        train_dataset, train_labels = reformat(np.array(x_train), np.array(y_train),num_labels,example_rows,example_columns)
    ...
    def fit_predict(self,x,y,example_rows,num_labels,is_train,**kwargs):
        #refit and predict on test
        if 'patience' in self.params:
            patience = self.params['patience']
            patience_increase = 2 #wait this much longer when a new best is found
